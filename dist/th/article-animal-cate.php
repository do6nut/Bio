        ...
        <div class="mb-7">
            ...
            <?php
                $sql = "SELECT * FROM article
                                  WHERE group_Type = 'ProductCategory'";
                $resultArticle = $conn->query($sql);
            ?>
            <?php while ($resultCheck = $resultArticle->fetch_assoc()) : ?>
                <?php
                $arr = '';
                $new_str = explode(',', $row['article_tag']);

                foreach($new_str as $arr){

                    if(($arr)!= ""){
                        if(($arr)== $resultCheck['article_Header']){  ?>
                        <a class="btn btn-soft-secondary btn-sm mb-1" href="./article-Product-cate.php?id=<?=  $resultCheck['article_id'] ?>">  <?= $arr ?></a>
                        <?php }?>
                    <?php }?>
                 <?php }?>
            <?php endwhile ?>
