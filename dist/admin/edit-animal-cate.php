                ...
                <div class="card-body p-sm-7 p-md-5">
                  ...
                  <div class="mb-3">
                      ...
                      <div class="row row-cols-2 row-cols-sm-2 row-cols-md-4">
                              ...
                              ORDER BY article_type, article_header ASC ";
                              $result = $conn->query($sql);

                              ?>
                              <?php while ($resultCheck = $result->fetch_assoc()) : ?>
                          <div class="col mb-5 mb-md-0">
                                  <input type="checkbox" name="tag[]" value="<?=$resultCheck['article_Header']?>"
                                      <?php
                                          $arr = '';
                                          $new_str = explode(',', $row['article_tag']);
                                          foreach($new_str as $arr){

                                          if(($arr)== $resultCheck['article_Header']){      echo $arr;?>

                                             checked="checked"
                                <?php echo strtoupper($arr);  }?>
                                      <?php }?>
                               > <?=$resultCheck['article_Header'] ?><br>
                          </div>
                              <?php endwhile ?>
